...
describe('#breedDetails', () => {
  ...
  it('returns undefined when a breed that\'s not included, or no aruguments at all is called', () => {
    const siamese = breedDetails('Siamese');
    assert.equal(siamese, undefined);

    const noArgs = breedDetails();
    assert.equal(noArgs, undefined);
  });
});